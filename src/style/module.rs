				...
				for nested in &items {
					match nested {
						ast::Item::Use(use_item) if first_non_use_start.is_none() => {
							...
							leading_use_origin =
								Some(use_item_origin(use_item, &local_module_roots));
						},
						_ => {
							first_non_use_start =
...
fn build_module_reorder_plans(
	...
	while idx < ctx.top_items.len() {
		...
		let mut run_mod001_lines = Vec::new();
		let mut last_bucket = None;

		for entry in &entries {
			if let Some(last) = last_bucket {
				if entry.bucket < last {
					run_mod001_lines.push(entry.line);
				}
			}
		...
		if run_mod001_lines.is_empty() && run_mod002_lines.is_empty() && run_mod003_lines.is_empty()
		{
			idx = end;
