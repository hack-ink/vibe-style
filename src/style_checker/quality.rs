...
use super::shared::{
	Edit, FileContext, SNAKE_CASE_RE, STD_QUALIFIED_MACRO_RE, Violation, line_from_offset,
	offset_from_line, push_violation, strip_string_and_line_comment, text_range_to_lines,
};
...
pub(crate) fn check_expect_unwrap(ctx: &FileContext, violations: &mut Vec<Violation>) {
...
pub(crate) fn check_numeric_literals(
	...
	for literal in ctx.source_file.syntax().descendants().filter_map(ast::Literal::cast) {
		...
		let line = line_from_offset(&ctx.line_starts, start);
