...
pub(crate) static SERDE_DEFAULT_RE: Lazy<Regex> =
	Lazy::new(|| Regex::new(r"^\s*#\s*\[\s*serde\s*\(\s*default\b[^)]*\)\s*]\s*$").unwrap());
pub(crate) static USE_RE: Lazy<Regex> =
	Lazy::new(|| Regex::new(r"^\s*(pub\s+)?use\s+(.+);\s*$").unwrap());
...
pub(crate) static STD_QUALIFIED_MACRO_RE: Lazy<Regex> = Lazy::new(|| {
